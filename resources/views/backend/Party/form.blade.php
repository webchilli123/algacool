...
{{-- <div class="card mb-3">
    <div class="card-header bg-primary text-white">
        <strong>Finished Items</strong>
    </div>
    <div class="card-body">
        <div class="row">
            <div class="col-md-12">
                <label class="form-label fw-bold">
                    Select Finished Items
                </label>

                <select
                    name="finished_item_ids[]"
                    id="finished_item_ids"
                    class="form-control select2"
                    multiple>

                    @foreach($finishedItemList as $id => $name)
                        <option value="{{ $id }}"
                            {{ in_array($id, old('finished_item_ids', $selectedFinishedItems)) ? 'selected' : '' }}>
                            {{ $name }}
                        </option>
                    @endforeach
